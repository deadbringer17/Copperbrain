...
| `src/copperbrain/adapters/freerouting.py` | Dynamic local runtime detection with verified-capability preference, hash-bound DSN preserve-class scope validation, incremental-safe splitting of KiCad multi-segment wire paths, fixed-command candidate generation, bounded per-pass progress parsing, watchdogs, and process cleanup | Autorouter discovery, routing scope, DSN wiring normalization, CLI capability policy, telemetry, watchdog, timeout, or strategy changes |
...
| `src/copperbrain/adapters/pcb_design.py` | Typed KiCad 9/10 PCB parser with copper-shape-aware pad/track/via/zone connectivity, placement and board-layer-validated segment/via writers, plus project-path-verified official KiCad IPC transactions | PCB geometry, connectivity, layer/net-format compatibility, placement/routing serialization, or live IPC behavior changes |
...
| `src/copperbrain/services/reference_design.py` | Bounded LM2596/LM5576 sections, the 120×100 mm brushed-motor benchmark, and the compact 85×50 mm four-layer DRV8311S BLDC benchmark, all expressed as typed operations/models | Reference topology, assumptions, placement, rules, or metadata changes |
| `src/copperbrain/server.py` | Thin FastMCP stdio entry point exposing 63 core and approved-extension tools, including safe project creation, post-placement grounding, persistent routing, metric readback, and finalization/readiness contracts | MCP tools or transport serialization change |
...
Real-project work follows the gated sequence documented in `AGENTS.md`: intake/baseline,
schematic and sourcing, PCB rules, placement, grounding/power structure, routing by explicit
net class and bounded batch, then final validation/readiness. Each mutation remains
`prepare -> preview -> explicit confirmation -> validate -> apply`, with source hashes, private
workspace, snapshot, atomic replacement, and rollback.
