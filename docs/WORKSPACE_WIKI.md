...
| `src/copperbrain/adapters/kicad_project_worker.py` | Fixed-action worker using KiCad's bundled Python to create/relayer boards and apply validated footprint move/rotate/flip manifests | Empty-board API creation, layer policy, or coordinated footprint transforms change |
...
| `src/copperbrain/server.py` | Thin FastMCP stdio entry point exposing 58 core and approved-extension tools, including safe project creation, persistent routing, and finalization/readiness contracts | MCP tools or transport serialization change |
...
unrelated nets; missing requested nets fail before Java starts.
