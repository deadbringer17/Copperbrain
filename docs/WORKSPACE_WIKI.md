...
| `src/copperbrain/adapters/kicad_board_worker.py` | Fixed-action worker executed by KiCad's bundled Python for post-route zone refill | Routed-zone behavior changes |
...
| `src/copperbrain/adapters/routing_backend.py` | Backend-neutral fixed-command contract, typed candidate result, and bounded strategy identifiers | Router service/adapter boundary changes |
| `src/copperbrain/adapters/kicad_routing_tools.py` | Managed KiCadRoutingTools discovery, platform Rust-core readiness, exact-net fixed CLI construction, `mps`/`inside_out`/`original` strategies, bounded sanitized output, attempt metrics, timeout/stall watchdog, process-tree cleanup, and routed-board zone refill | Autorouter discovery, routing scope, attempt configuration, telemetry, watchdog, timeout, or runtime contract changes |
...
| `src/copperbrain/services/pcb_routing.py` | KiCadRoutingTools orchestration, rule-first net roles, single-candidate default (up to three orderings on request) ranking and best-only prepared-PCB application, zone-derived default plane-net exclusion, opt-in geometry-derived fine-pitch stubs and opposite-layer dogbones, local routing-hotspot detection, diagnostic-only partial candidates, safe scoped copper deltas, candidate/lifecycle metrics, compact review, preview, restart-safe apply/rollback, and snapshot recovery | PCB routing scope gates, attempt cap, plane-exclusion defaults, escape policy, role policy, hotspot/placement evidence, metrics, ranking, persistence, recovery, or workflow changes |
...
| `scripts/setup_dependencies.py` | Explicit, checksum-verified installer for the managed KiCadRoutingTools PCM release/platform Rust core plus optional JLC plugins from official sources only; never runs silently or through MCP | Dependency sources, checksum policy, target directories, platform-core selection, or CLI flags change |
| `tests/adapters/test_kicad_routing_tools.py` | Managed-runtime discovery/status, exact bounded command scope, log sanitization, structured failure metrics, and strategy-cap tests | KiCadRoutingTools adapter or provisioning contract changes |
